...
@implementation SPTObjectProperty
...
- (instancetype)initWithName:(NSString *)name typeString:(NSString *)typeString {
    ...
    if (self != nil) {
        ...
        if ([typeStringComponents count] > 0) {
            ...
            NSString *typeInfo = typeStringComponents[0];
            ...
            if ([typeInfo length] > scanLocation) {
                ...
                NSNumber *indexNumber = [self.class encodedTypesMap][typeCode];
                self.propertyType = (SPTObjectPropertyType)[indexNumber unsignedIntegerValue];
                
                //当当前的类型为对象的时候，解析出对象对应的类型的相关信息
                //T@"NSArray<QSMyModel>"
                if (self.propertyType == SPTObjectPropertyTypeObject) {
